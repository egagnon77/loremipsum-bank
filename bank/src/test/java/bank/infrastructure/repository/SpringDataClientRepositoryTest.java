...
public class SpringDataClientRepositoryTest {
    ...
    private static final String A_VALID_ID = "aValidId";
    private static final String AN_INVALID_ID = "anInvalidId";
    ...
    public void givenAValidId_whenFindById_thenAClientMustBeReturned() {
        ...
        when(crudClientRepository.findById(A_VALID_ID)).thenReturn(optionalClientDto);
        ...
        Optional<Client> result = testedClass.findById(A_VALID_ID);
    ...
    public void givenAnInvalidId_whenFindById_thenAClientMustNotBeReturned() {
        when(crudClientRepository.findById(AN_INVALID_ID)).thenReturn(Optional.empty());
        Optional<Client> optionalClient = Optional.empty();
        when(clientMapper.toClient(Optional.empty())).thenReturn(optionalClient);

        Optional<Client> result = testedClass.findById(AN_INVALID_ID);
